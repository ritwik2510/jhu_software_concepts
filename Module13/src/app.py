...
from src import query_data, load_data, inference
...
def create_app(test_config=None):
    ...
    def predict():
        """Render the Will You Get In? prediction page."""
        result = None
        error = None
        ...
        if request.method == "POST":
            try:
                ...
                gre = parse_float(request.form.get("gre", ""))
                gre_v = parse_float(request.form.get("gre_v", ""))
                gre_aw = parse_float(request.form.get("gre_aw", ""))

                result = inference.predict(
                    program, term, degree, citizenship,
                    gpa, gre, gre_v, gre_aw, comments
                )
            except Exception:
                error = "Something went wrong processing your prediction. Please check your inputs and try again."
