...
from publisher import publish_task  # ✅ replaces the load_data/query_data imports
...
def get_connection():
    ...
    if not db_url:
        ...
        name = os.getenv("DB_NAME", "applicants")   # ✅ changed gradcafe → applicants
        user = os.getenv("DB_USER", "appuser")       # ✅ changed postgres → appuser
        password = os.getenv("DB_PASSWORD", "apppass")  # ✅ matches docker-compose
    ...
    def analysis():
        ...
        q3 = safe_get(
            sql.SQL("""
                ...
                FROM applicants WHERE gpa IS NOT NULL LIMIT %s
            """),
            (safe_limit(1),),
        ...
        q4 = safe_get(
            sql.SQL("""
                ...
                FROM applicants
                WHERE term = %s AND gpa IS NOT NULL
                AND (
        ...
        q5 = safe_get(
            sql.SQL("""
                SELECT ROUND(
                    ...
                    / NULLIF(COUNT(*),0),2)
                FROM applicants WHERE term = %s LIMIT %s
            """),
            ("Fall 2026", safe_limit(1)),
            default=(0,),
        )
        q6 = safe_get(
            sql.SQL("""
                SELECT ROUND(AVG(gpa)::numeric,2)
                FROM applicants
                WHERE status ILIKE '%accept%' AND term = %s LIMIT %s
            """),
            ("Fall 2026", safe_limit(1)),
            default=(None,),
        )
        ...
        q8 = safe_get(
            sql.SQL("""
                SELECT COUNT(*) FROM applicants
                WHERE status ILIKE '%accept%' AND term = %s
                AND (
        ...
        q10 = fetch(
            sql.SQL("""
                ...
                GROUP BY status ORDER BY AVG(gpa) DESC LIMIT %s
            """),
            (safe_limit(100),),
        ) or []
        q11 = fetch(
            sql.SQL("""
                ...
                FROM applicants GROUP BY program
        ...
        return render_template(
            "index.html",
            q1=q1[0], q2=q2[0],
            q3_gpa=q3[0], q3_gre=q3[1], q3_grev=q3[2], q3_aw=q3[3],
            q4=q4[0], q5=q5[0], q6=q6[0], q7=q7[0], q8=q8[0],
            q9_programs=q9[0][0], q9_universities=q9[0][1],
            q10=q10, q11=q11,
        )
